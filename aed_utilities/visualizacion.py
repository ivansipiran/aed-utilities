...
class LinkedListDrawer:
    ...
    def draw_linked_list(self, nList):
        if self.strHeader != "":
            listStr = 'node[shape=plaintext];'+self.strHeader + \
                '; node[shape=square]; ""; node[shape=circle]; ' + \
                self.strHeader + '-> '
        else:
    ...
    def draw_double_linked_list(self, nList):
        ...
        src = Source(
            'digraph "Lista" { rankdir=LR; ' + listStrAsc + ' ' + listStrDesc + ' }')
        src.render('lista.gv', view=True)
        display(SVG(src.pipe(format='svg')))
...
class BinaryTreeDrawer:
                ...
                if not hasattr(node, "info"):
                    if not self.drawNull:
                        ...
                        newNode = PositionNode(
                            None, "", None, "null", "null" + str(self.counterNull))
                    ...
                    if not self.drawNull:
                        ...
                        newNode1 = PositionNode(
                            None, "", None, "null", "null" + str(self.counterNull))
                        self.counterNull = self.counterNull + 1
                        newNode2 = PositionNode(
                            None, "", None, "null", "null" + str(self.counterNull))
                        self.counterNull = self.counterNull + 1
    ...
    def encode_nodes(self, node):
        ...
        for item in L:
            if "null" in str(item[3]):
                listStr = listStr + ' ' + str(item[3]) + '[pos="' + str(item[1]) + ',' + str(
                    item[2]) + '!" shape=square label="'+str(item[0])+'" width="0.2"] '
            else:
                listStr = listStr + '"' + str(item[3]) + '"' + '[pos="' + str(
                    item[1]) + ',' + str(item[2]) + '!" label="'+str(item[0])+'" shape=circle] '
    ...
    def encode_edges(self, node):
        ...
        if node.left is not None:
            listStr = listStr + " " + \
                str(node.code) + "--" + str(node.left.code)
            listStr = listStr + self.encode_edges(node.left) + " "

        if node.right is not None:
            listStr = listStr + " " + \
                str(node.code) + "--" + str(node.right.code)
    ...
    def draw_tree(self, tree, root):
        ...
        src = Source('graph "Arbol" { rankdir=TB; ' +
                     listNodes + ' node[shape=circle] ' + listStr + ' }')
        src.engine = "neato"
        src.render('lista.gv', view=True)
        display(SVG(src.pipe(format='svg')))
...
class NumpyArrayDrawer:
    ...
    def drawNumpy1DArray(self, array, showIndex=False, layout="row"):
        ...
        if layout == "row":
            strArray = "<TR>"
            for i in range(array.shape[0]):
                strArray = strArray + '<TD border="1" fixedsize="true" width="' + \
            ...
            if showIndex:
                strArray = strArray + "<TR>"
                for i in range(array.shape[0]):
                    strArray = strArray + '<TD border="0" fixedsize="true" width="' + \
                        str(size)+'" height="'+str(size) + \
                        '">' + str(i) + '</TD>'
                strArray = strArray + '</TR>'
        elif layout == "column":
            ...
            for i in range(array.shape[0]):
                if not showIndex:
                    strArray = strArray + '<TR><TD border="1" fixedsize="true" width="' + \
                        str(size)+'" height="'+str(size) + \
                        '">' + str(array[i]) + '</TD></TR>'
                else:
                    strArray = strArray + '<TR><TD border="0" fixedsize="true" width="'+str(size)+'" height="'+str(size)+'">' + str(
                        i) + '</TD><TD border="1" fixedsize="true" width="'+str(size)+'" height="'+str(size)+'">' + str(array[i]) + '</TD></TR>'

        src = Source(
            'graph "Array" { node [fontsize=15, shape=plaintext]; a0 [label=< <TABLE border="0" cellspacing="0" cellpadding="3">' + strArray + '</TABLE> >] }')
        src.render('lista.gv', view=True)
        display(SVG(src.pipe(format='svg')))
    ...
    def drawNumpy2DArray(self, array, showIndex=False):
        ...
        if showIndex:
            strArray = strArray + '<TR><TD border="0" fixedsize="true" width="' + \
                str(size)+'" height="'+str(size)+'"></TD>'
            for j in range(array.shape[1]):
                strArray = strArray + '<TD border="0" fixedsize="true" width="' + \
                    str(size)+'" height="'+str(size)+'">' + str(j) + '</TD>'
            strArray = strArray + '</TR>'

        for i in range(array.shape[0]):
            if showIndex:
                strArray = strArray + '<TR><TD border="0" fixedsize="true" width="' + \
                    str(size)+'" height="'+str(size)+'">' + str(i) + '</TD>'
            else:
                strArray = strArray + '<TR>'
            for j in range(array.shape[1]):
                strArray = strArray + '<TD border="1" fixedsize="true" width="' + \
